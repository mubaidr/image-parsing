...
class WorkerManager extends events_1.EventEmitter {
    ...
    createWorkers(count, type) {
        for (let i = 0; i < count; i += 1) {
            const worker = child_process_1.fork(`./dist-electron/workers/${type}.worker.js`);
    ...
    async compile(resultPath, keyPath, correctMarks, incorrectMarks) {
        const results = CompiledResult_1.default.loadFromExcel(resultPath).getResults();
...
async function readKey(src) {
    ...
    const designData = await design_1.getDesignData(dataPaths_1.dataPaths.designBarcode);
